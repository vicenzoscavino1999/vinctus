            ...
            <div className="relative w-full max-w-2xl mx-4 bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden">
                ...
                <div className="p-6 space-y-5">
                    <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-4">
                        <p className="text-xs uppercase tracking-widest text-neutral-500">{collaboration.context}</p>
                        <h3 className="text-2xl text-white font-serif font-light mt-2">{collaboration.title}</h3>
                        <p className="text-sm text-neutral-500 mt-1">
                            Por <span className="text-amber-200/80">{collaboration.authorSnapshot.displayName}</span>
                        </p>
                    </div>

                    <div className="grid gap-3 md:grid-cols-2 text-sm text-neutral-400">
                        <div>
                            <span className="text-neutral-500 uppercase tracking-widest text-xs block mb-1">Que busca</span>
                            <span className="text-white">{collaboration.seekingRole}</span>
                        </div>
                        <div>
                            <span className="text-neutral-500 uppercase tracking-widest text-xs block mb-1">Modalidad</span>
                            <span className="text-white">{formatMode(collaboration.mode)}</span>
                        </div>
                        {collaboration.location && (
                            <div>
                                <span className="text-neutral-500 uppercase tracking-widest text-xs block mb-1">Ubicacion</span>
                                <span className="text-white">{collaboration.location}</span>
                            </div>
                        )}
                        <div>
                            <span className="text-neutral-500 uppercase tracking-widest text-xs block mb-1">Nivel</span>
                            <span className="text-white">{formatLevel(collaboration.level)}</span>
                        </div>
                        {collaboration.topic && (
                            <div className="md:col-span-2">
                                <span className="text-neutral-500 uppercase tracking-widest text-xs block mb-1">Tema especifico</span>
                    ...
                    {collaboration.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            {collaboration.tags.map((tag) => (
                                <span key={tag} className="text-[10px] text-neutral-400 bg-neutral-800/70 px-2 py-1 rounded">
                                    {tag}
                                </span>
                            ))}
                        </div>
                    ...
                    {!isOwner && (
                        <form onSubmit={handleSubmit} className="space-y-3">
                            ...
                            <div className="flex items-center justify-between text-xs text-neutral-500">
                                <span>{isRequested ? 'Solicitud enviada' : 'Envias un solo mensaje junto a la solicitud.'}</span>
