...
import { createStory, type StoryMediaType } from '../lib/firestore';
import { uploadStoryImage, uploadStoryVideo } from '../lib/storage';
    ...
    const prepareImage = async (file: File): Promise<StoryDraft | null> => {
        ...
        if (isHeicFile(file)) {
            try {
                ...
                sourceFile = new File([blob as Blob], replaceExtension(file.name, 'jpg'), { type: 'image/jpeg' });
            } catch (error) {
        ...
        try {
            ...
            const compressedFile = new File(
                [compressedBlob],
                replaceExtension(sourceFile.name, 'webp'),
                { type: 'image/webp' }
            );
            ...
            const thumbFile = new File(
                [thumbBlob],
                replaceExtension(sourceFile.name, 'thumb.webp'),
                { type: 'image/webp' }
            );
        ...
        try {
            ...
            if (draft.type === 'image') {
                const result = await uploadStoryImage(
                    ...
                    storyId,
                    (progressValue) => {
                        const pct = progressValue.totalBytes > 0
