        ...
        >
            <div className="flex items-center gap-3 text-left">
                <div className="w-10 h-10 rounded-full bg-neutral-900 border border-neutral-800 overflow-hidden flex items-center justify-center text-neutral-500 text-sm">
                    {entry.photoURL ? (
                        <img src={entry.photoURL} alt={entry.displayName ?? 'Usuario'} className="w-full h-full object-cover" />
                    ) : (
            ...
            <span className="text-xs text-neutral-500">{activeTab === 'followers' ? 'Seguidor' : 'Siguiendo'}</span>
        </button>
    );
                ...
                <div className="inline-flex bg-neutral-900/30 border border-neutral-800 rounded-full p-1">
                    <button
                        ...
                        className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${
                            activeTab === 'followers' ? 'bg-neutral-800 text-white' : 'text-neutral-500 hover:text-neutral-300'
                        }`}
                    >
                        Seguidores
                    </button>
                    <button
                        ...
                        className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${
                            activeTab === 'following' ? 'bg-neutral-800 text-white' : 'text-neutral-500 hover:text-neutral-300'
                        }`}
                    >
                        Seguidos
                    </button>
                ...
                <section className="mb-8">
                    ...
                    <div className="bg-[#1A1A1A] border border-neutral-800 rounded-2xl overflow-hidden">
                        {loadingRequests ? (
                            ...
                            requests.map((req) => (
                                <div key={req.requestId} className="flex items-center justify-between px-4 py-3 border-b border-neutral-800 last:border-0">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-neutral-900 border border-neutral-800 overflow-hidden flex items-center justify-center text-neutral-500 text-sm">
                                            {req.photoURL ? (
                                                <img src={req.photoURL} alt={req.displayName ?? 'Usuario'} className="w-full h-full object-cover" />
                                            ) : (
                                        ...
                                        <div>
                                            ...
                                            {req.username && <div className="text-xs text-neutral-500">@{req.username}</div>}
                                        </div>
                                    </div>
            ...
            <section>
                <div className="bg-[#1A1A1A] border border-neutral-800 rounded-2xl overflow-hidden">
                    {loading ? (
                        ...
                        <div className="text-sm text-red-400 text-center py-10">
                            {error}
                        </div>
                    ) : items.length === 0 ? (
