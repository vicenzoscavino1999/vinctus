...
import { Search, ChevronRight, Users, ArrowLeft, Send, X, ExternalLink, Pencil, LogOut, Bell, BellOff, Share2, Paperclip, FileText, Image as ImageIcon, Flag } from 'lucide-react';
...
export default function MessagesPage() {
    ...
    const [groupOptionsGroup, setGroupOptionsGroup] = useState<FirestoreGroup | null>(null);
    ...
    const handleOpenGroupOptions = async () => {
        ...
        const cached = memberGroups.find((g) => g.id === groupId);
        if (cached) {
            setGroupOptionsGroup(cached);
        }

        if (user?.uid && selectedConversationId) {
            try {
                const member = await getConversationMember(selectedConversationId, user.uid);
                if (member?.muted && member.mutedUntil && member.mutedUntil < new Date()) {
                    await clearConversationMute(selectedConversationId, user.uid);
                    setGroupMemberData({ ...member, muted: false, mutedUntil: null });
                } else {
                    setGroupMemberData(member);
                }
            } catch (error) {
                console.error('Error loading group member data:', error);
            }
        }
        ...
        if (!cached) {
            ...
            try {
                ...
                setGroupOptionsGroup(group);
            } catch (error) {
                console.error('Error loading group for options:', error);
            } finally {
    ...
    const handleClearChatLocal = () => {
        if (!selectedConversationId) return;
        const confirmed = window.confirm('¿Limpiar el chat localmente? Esto no borra mensajes del servidor.');
        if (!confirmed) return;
    ...
    if (selectedConversationId && activeConversation) {
        const clearedAt = selectedConversationId ? clearedConversations[selectedConversationId] : undefined;
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center">
                        ...
                        <div className="relative w-full max-w-md mx-4 bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden">
                            <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-800">
                                <h3 className="text-lg font-medium text-white">Detalles del grupo</h3>
                                <button
                                    onClick={() => setShowGroupOptions(false)}
                            ...
                            <div className="flex flex-col items-center py-6 border-b border-neutral-800/50">
                                <div className="w-20 h-20 rounded-full bg-neutral-800 border border-neutral-700 overflow-hidden flex items-center justify-center mb-4">
                                    {groupOptionsGroup?.iconUrl ? (
                                        ...
                                        <span className="text-2xl text-neutral-400">
                                            {(groupOptionsGroup?.name || getConversationName(activeConversation) || 'G').charAt(0).toUpperCase()}
                                        </span>
                                    )}
                                </div>
                            ...
                            <div className="p-4 space-y-4">
                                ...
                                <div className="max-h-[45vh] overflow-y-auto space-y-2">
                                    {searchConversationQuery.trim() === '' ? (
                                        ...
                                        visibleMessages
                                            .filter((msg) => msg.text.toLowerCase().includes(searchConversationQuery.trim().toLowerCase()))
                                            .map((msg) => (
                                                <button
                                                    key={msg.id}
                                                    type="button"
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="w-12 h-12 rounded-lg bg-neutral-800 flex items-center justify-center overflow-hidden">
                                                    {isImage ? (
                                                        att.thumbUrl ? (
                                                            <img src={att.thumbUrl} alt={title} className="w-full h-full object-cover" />
                                                        ) : (
