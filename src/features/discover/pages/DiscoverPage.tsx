            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] uppercase tracking-wide transition-colors ${
                    isCategorySaved(cat.id)
              ...
              <div className="flex flex-wrap gap-2 pt-3 border-t border-neutral-800/50">
                {cat.subgroups.slice(0, 2).map((sub) => (
                  <span
                    key={sub.id}
                    className="text-[10px] uppercase tracking-wider text-neutral-500 bg-neutral-800/50 px-2 py-1 rounded"
                  >
                    {sub.name}
                  </span>
                ))}
          ...
          {PUBLICATIONS.map((pub) => (
            ...
            >
              ...
              <div className="absolute inset-0 p-5 flex flex-col justify-between">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-amber-500/20 border border-amber-500/30 flex items-center justify-center">
                    {(() => {
                      ...
                      return cat ? (
                        <cat.icon size={14} className="text-amber-200" strokeWidth={1.5} />
                      ) : null;
                    })()}
                  </div>
                  <div>
                ...
                <div className="absolute right-5 top-1/2 -translate-y-1/2 flex flex-col gap-4">
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center mb-1">
                      <svg
                        className="w-5 h-5"
                        fill={isPostLiked(pub.id) ? 'currentColor' : 'none'}
                        stroke="currentColor"
                        strokeWidth="2"
                        viewBox="0 0 24 24"
                      >
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center">
                      <svg
                        className="w-5 h-5"
                        fill={isPostSaved(pub.id) ? 'currentColor' : 'none'}
                        stroke="currentColor"
                        strokeWidth="2"
                        viewBox="0 0 24 24"
                      >
                ...
                {/* Bottom - Title */}
                <div>
                  <h3 className="text-white text-lg font-light mb-2 pr-16 line-clamp-2">
                    {pub.title}
                  </h3>
